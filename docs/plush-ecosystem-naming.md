...
| Per-task pause (`tracker_tasks.paused_since`/`paused_until`) + Path pause (`plush_path_progress.status`) | PlushPause | `index.html` `isTaskPausedOnDate`, `pauseTrackerTask`/`resumeTrackerTask`, "Change my tasks" panel; `pauseCarePath` | Applied | A single habit/routine can now be paused (Change my tasks → ⏸ Pause/▶️ Resume) for a date range, excluded from required/possible counts on paused dates without affecting past history (`paused_since`/`paused_until` are a stored range, not a live boolean — resuming caps the range rather than clearing it, so already-elapsed paused days don't retroactively look missed). Paused-task reminders are suppressed. Paths already had a pause toggle (`plush_path_progress.status`); it's now shown with a visible "⏸ PAUSED" badge. Whole-day Rest Days / Vacation Mode is unchanged and still the right tool for pausing everything at once — PlushPause is for pausing one thing. |
