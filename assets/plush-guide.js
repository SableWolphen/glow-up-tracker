(function () {
  ...
  // Replace any older PlushGuide instance cleanly when a new web build loads.
  ...
  const FEATURES = {
      ...
      icon: "🧸", name: "PlushRescue", dashboard: "today",
      summary: "Make today smaller, choose one next step, or soften pressure.",
      what: "PlushRescue helps when the full day feels like too much. It makes the next step feel smaller without deleting your original tasks.",
      when: "Use it when you feel stuck, overloaded, behind, or need a gentler version of today.",
      openLabels: ["plushrescue"],
      tour: [
    ...
    focus: {
      icon: "🎯", name: "PlushFocus", dashboard: "today",
      summary: "See one clear task without the rest of the day crowding you.",
      what: "PlushFocus keeps the rest of the list out of the way and puts one useful next task in front of you.",
      when: "Use it when seeing everything at once makes it harder to start.",
      openLabels: ["plushfocus", "focus"],
      tour: [
        { title: "One thing at a time", text: "PlushFocus intentionally keeps the rest of the list out of the way so you can concentrate on the next useful step.", labels: ["plushfocus", "focus mode"], fallback: "dashboard-tab-today" },
    ...
    calendar: {
      icon: "📅", name: "PlushCalendar", dashboard: "calendar",
      summary: "See Month, Week, and Day views.",
      what: "PlushCalendar lets you look ahead at routines and tasks by date instead of only seeing today.",
      when: "Use it to understand what is coming up, spot busy days, or review a week or month.",
      tour: [
    ...
    progress: {
      icon: "📈", name: "PlushProgress", dashboard: "progress",
      summary: "Review trends, consistency, and what is helping.",
      what: "PlushProgress turns completed routines into a gentle history of consistency and patterns. It is for noticing progress, not grading yourself.",
      when: "Use it when you want to see what has been working over time.",
      tour: [
    ...
    calm: {
      icon: "🌙", name: "PlushCalm", dashboard: "care",
      summary: "Open grounding, sound, breathing, and gentle-care tools.",
      what: "PlushCalm collects short calming and grounding tools in one place.",
      when: "Use it when you want a breathing exercise, grounding prompt, comforting sound, or another low-pressure care tool.",
      openLabels: ["plushcalm", "calm"],
      tour: [
    ...
    journal: {
      icon: "📖", name: "PlushJournal", dashboard: "today",
      summary: "Write a private reflection or return to today’s prompt.",
      what: "PlushJournal gives you a private prompt and space to write.",
      when: "Use it when you want to capture what happened or reflect on how you feel.",
      tour: [
    ...
    paths: {
      icon: "🌿", name: "PlushPaths", dashboard: "care",
      summary: "Open guided routines and care paths.",
      what: "PlushPaths are guided, multi-step experiences that help you work through a routine or care goal over time.",
      when: "Use one when you want more guidance than a single task.",
      openLabels: ["plushpaths", "paths"],
      tour: [
    ...
    tasks: {
      icon: "✅", name: "Change My Tasks", dashboard: "today",
      summary: "Add, edit, pause, schedule, or clean up routines.",
      what: "This is the control center for your PlushList.",
      when: "Use it whenever your real life changes and the list needs to change with you.",
      openLabels: ["change my tasks", "manage tasks", "edit tasks"],
      tour: [
        ...
        { title: "Add or edit", text: "Create something new or edit an existing routine when it needs to change.", labels: ["add", "edit", "new task"] },
    ...
    guardian: {
      icon: "🤝", name: "PlushGuardian", profile: true,
      summary: "Open the support connection and privacy controls.",
      what: "PlushGuardian is the optional support connection area.",
      when: "Use it to review or change a support connection, permissions, or privacy choices.",
      openLabels: ["add a guardian", "plushguardian", "guardian"],
      tour: [
    ...
    safety: {
      icon: "🛟", name: "PlushSafety", profile: true,
      summary: "Find safety resources and account protections.",
      what: "PlushSafety gathers safety resources so they are easier to find when you need them.",
      when: "Use it when you want to review the safety resources available inside PlushLife.",
      openLabels: ["plushsafety"],
      tour: [
  ...
  const labelText = (node) => clean(node && (node.getAttribute("aria-label") || node.getAttribute("title") || node.textContent));
  ...
  function closeProfileIfNeeded() {
    ...
    const close = Array.from(profile.querySelectorAll('button,[role="button"]')).find((node) => {
      const text = labelText(node);
  ...
  async function openDashboard(id) {
    ...
    return !!document.getElementById(`dashboard-tab-${id}`)?.getAttribute("aria-selected");
  }
  ...
  function findTarget(labels, fallbackId) {
    ...
    for (const node of candidates) {
      ...
      if (wanted.has(labelText(node))) return node;
    }
    if (fallbackId) {
  ...
  async function positionSpotlight(tour, target) {
    ...
    if (!Number.isFinite(rect.top) || rect.width < 16 || rect.height < 16) return false;
  ...
  async function renderTourStep(featureKey, index) {
    ...
    await nextPaint();
    const target = findTarget(step.labels, step.fallback || (safeIndex === 0 && feature.dashboard ? `dashboard-tab-${feature.dashboard}` : null));
    if (target) await positionSpotlight(tour, target);
  }
  ...
  async function route(featureKey) {
    ...
    if (feature.profile) {
      ...
      const button = exactClickable(feature.openLabels, panel || document);
    ...
    const dashboardOpened = await openDashboard(feature.dashboard);
    if (!dashboardOpened) return;

    if (feature.openLabels && feature.openLabels.length) {
      const button = exactClickable(feature.openLabels);
      if (button && !button.id?.startsWith("dashboard-tab-")) {
        button.click();
        await nextPaint();
      }
    }
  ...
  function itemMarkup(key) {
    ...
    return `<div class="pg-item" data-feature="${key}"><button class="pg-action" type="button" aria-expanded="false">${feature.icon} ${feature.name}<span class="pg-chevron">⌄</span><small>${feature.summary}</small></button><div class="pg-details"><strong>What it does</strong>${feature.what}<strong>When it can help</strong>${feature.when}<button class="pg-show" type="button">Show me →</button></div></div>`;
  }
  ...
  function openGuide() {
    ...
    guide.querySelectorAll(".pg-item").forEach((item) => {
      ...
      action.addEventListener("click", () => {
        const opening = action.getAttribute("aria-expanded") !== "true";
        guide.querySelectorAll('.pg-action[aria-expanded="true"]').forEach((other) => {
          other.setAttribute("aria-expanded", "false");
          other.parentElement.querySelector(".pg-details").classList.remove("open");
        });
        action.setAttribute("aria-expanded", opening ? "true" : "false");
        details.classList.toggle("open", opening);
      });
      item.querySelector(".pg-show").addEventListener("click", (event) => {
  ...
  function installEntry() {
    ...
    if (first && first.parentElement) first.parentElement.insertBefore(entry, first);
