(function () {
  ...
  const STORAGE_KEY = "plushlife-rescue-v5";
  ...
  const openPanel = () => {
    ...
    panel.querySelector('[data-action="next"]').addEventListener("click", () => {
      const count = showOnly(1, true, true); if (!count) return note("Open Home where your tasks are visible, then try again.");
    ...
    panel.querySelector('[data-action="pause"]').addEventListener("click", () => {
      document.body.classList.add("plushlife-pressure-paused"); saveState({ mode: "pause", pressureDate: TODAY, reentry: true }); launcher.textContent = "🌿 Rescue active"; finish("Pressure is softened for today.");
    });
  ...
  function startThunderstorm(button) {
    ...
    const rumble = () => {
      ...
      osc.type = "sawtooth"; osc.frequency.setValueAtTime(62 + Math.random() * 28, ctx.currentTime); osc.frequency.exponentialRampToValueAtTime(28, ctx.currentTime + 2.4);
      filter.type = "lowpass"; filter.frequency.value = 130;
      gain.gain.setValueAtTime(0.0001, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.7, ctx.currentTime + 0.05); gain.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + 2.8);
      osc.connect(filter); filter.connect(gain); gain.connect(master); osc.start(); osc.stop(ctx.currentTime + 3); nodes.push(osc, filter, gain);
  ...
  document.addEventListener("click", (event) => {
    ...
    } else if (thunder && /ocean|white noise|calm tone/.test(text)) {
      stopThunderstorm();
    }
  }, true);
